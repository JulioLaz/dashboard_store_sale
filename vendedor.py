...
def seller_pie(df):
   ...
   #  df_vendedores = df.groupby('nombre_vendedor').agg(
   #      total_vendido=('valor_total', lambda x: (x * df.loc[x.index, 'cantidad']).sum()),
   ...
   #  ).sort_values(by='total_vendido', ascending=False).reset_index()
    ...
    fig.update_traces(
        pull=[0.03,0,0,0,0,0],
        textposition='inside',
        texttemplate='%{percent:.1%}<br>%{label}<br>%{customdata[0]}',
        textfont_size=14,
